...
def run_inference(frame_bgr: np.ndarray, session_id: str, db, timestamp_ms: int = 0, video_id: Optional[int] = None, frame_number: int = 0) -> dict:
    ...
    # 3. Insert frame (handles duplicates gracefully with ON CONFLICT)
    h, w = frame_bgr.shape[:2]
    frame_id: Optional[int] = None
    try:
        frame_id = queries.insert_frame(
            conn=db, session_id=session_id, frame_number=frame_number, width=w, height=h,
            timestamp_ms=timestamp_ms,
        )
    except Exception as exc:
        logger.error("Failed to insert frame: %s", exc)

    # 4. Insert detections (only if frame_id was successfully obtained)
    if frame_id is None:
        logger.error("Cannot insert detections: frame_id is None")
    else:
        for det in detections:
            try:
                ...
                logger.error("Failed to insert detection for frame_id=%s: %s", frame_id, exc)
    ...
    if video_id is not None:
        ...
        for det in detections:
            try:
                ...
                logger.error("Failed to insert video detection for video_id=%s, frame=%s: %s", 
                           video_id, frame_number, exc)
