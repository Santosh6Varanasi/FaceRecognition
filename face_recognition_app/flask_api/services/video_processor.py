...
from services.annotated_video_generator import AnnotatedVideoGenerator
...
class VideoProcessorService:
    ...
    ANNOTATED_FOLDER = 'video_uploads/annotated'
    ...
    def upload_video(self, file: FileStorage, user_id: Optional[int] = None) -> VideoMetadata:
        ...
        try:
            # Calculate file hash for tracking (but don't use for deduplication)
            video_hash = self._calculate_file_hash(file_path)
            
            # Always insert as new video - removed deduplication check
            # Users may want to process the same video multiple times
    ...
    def generate_annotated_video(
        ...
        """
        Generate annotated video with bounding boxes burned into frames.
        ...
        Returns:
            Path to generated annotated video file
            
        Raises:
            ValueError: If original video cannot be opened
            IOError: If output path is not writable
            Exception: If video generation fails
        
        Requirements: 3.1, 4.2, 4.4
        """
        logger.info(f"Generating annotated video for video_id={video_id}")
        
        try:
            # Create annotated video generator
            generator = AnnotatedVideoGenerator(self.db)
            
            # Generate annotated video
            result = generator.generate(video_id, original_path, output_path)
            
            logger.info(
                f"Annotated video generated successfully for video_id={video_id}: "
                f"{result['frame_count']} frames in {result['duration_seconds']:.2f}s"
            )
            
            return result['output_path']
            
        except Exception as e:
            logger.exception(f"Failed to generate annotated video for video_id={video_id}")
            raise
    ...
    def process_video(self, video_id: int, job_id: str, file_path: str) -> None:
        ...
        try:
            ...
            try:
                ...
                # Construct output path for annotated video
                original_filename = os.path.basename(file_path)
                name_without_ext = os.path.splitext(original_filename)[0]
                annotated_filename = f"annotated_{video_id}_{uuid.uuid4()}.mp4"
                annotated_path = os.path.join(self.ANNOTATED_FOLDER, annotated_filename)
                
                # Generate annotated video
                self.generate_annotated_video(video_id, file_path, annotated_path)
                
                # Update database with annotated video path
                queries.update_annotated_video_path(self.db, video_id, annotated_path)
                
                logger.info(f"Annotated video path stored in database for video_id={video_id}: {annotated_path}")
                
            except Exception as gen_exc:
                logger.error(f"Failed to generate annotated video for video_id={video_id}: {str(gen_exc)}")
                # Don't fail the entire job if annotation generation fails
                # The video is still processed, just without the annotated version
                queries.update_video_job_status(
                    self.db, job_id, 'completed',
                    message=f'Processing complete but annotated video generation failed: {str(gen_exc)}'
                )
            
            # Generate timeline entries from video detections
            try:
                logger.info(f"Generating timeline entries for video_id={video_id}")
                timeline_entries = self.get_timeline(video_id)
                logger.info(f"Generated {len(timeline_entries)} timeline entries for video_id={video_id}")
            except Exception as timeline_exc:
                logger.error(f"Failed to generate timeline entries for video_id={video_id}: {str(timeline_exc)}")
                # Don't fail the entire job if timeline generation fails
    ...
    def reprocess_video(self, video_id: int, model_version: Optional[int] = None) -> Dict:
        ...
        try:
            # Get old annotated video path before deletion
            old_annotated_path = queries.get_annotated_video_path(self.db, video_id)
            ...
            # Delete old annotated video file if it exists
            if old_annotated_path and os.path.exists(old_annotated_path):
                try:
                    os.remove(old_annotated_path)
                    logger.info(f"Deleted old annotated video: {old_annotated_path}")
                except Exception as e:
                    logger.warning(f"Failed to delete old annotated video {old_annotated_path}: {str(e)}")
    ...
    def delete_video_with_files(self, video_id: int) -> None:
        """
        Delete video record and associated files from storage.
        
        This method implements the complete video deletion workflow:
        1. Retrieve video metadata including file paths
        2. Delete original video file from storage
        3. Delete annotated video file from storage (if exists)
        4. Delete video record from database (cascade deletes related records)
        
        Args:
            video_id: Database ID of the video to delete
        ...
        annotated_path = queries.get_annotated_video_path(self.db, video_id)
        
        logger.info(f"Deleting video {video_id} and associated files")
        ...
        try:
            ...
            if original_path and os.path.exists(original_path):
                try:
                    ...
                    logger.info(f"Deleted original video file: {original_path}")
                except Exception as e:
                    logger.warning(f"Failed to delete original video {original_path}: {str(e)}")
            ...
            if annotated_path and os.path.exists(annotated_path):
                try:
                    ...
                    logger.warning(f"Failed to delete annotated video {annotated_path}: {str(e)}")
            
            # Delete video record from database (cascade deletes related records)
            queries.delete_video(self.db, video_id)
            logger.info(f"Deleted video record {video_id} from database")
            
        except Exception as e:
            logger.exception(f"Failed to delete video {video_id}")
            raise
