...
def label_face(face_id: int):
    ...
    source_image_path = face_record.get("source_image_path", "")
    
    # Label the unknown face
    queries.label_unknown_face(db, face_id, person_id, labeled_by)
    
    # Insert into faces table with proper image saving
    ...
    try:
        ...
        for entry in labels:
            ...
            try:
                ...
                # Get face embedding and ID
                cursor.execute(
                    "SELECT embedding, id FROM unknown_faces WHERE id = %s", (face_id,)
                )
                ...
                # Save face image and insert into faces table
                from services import image_service
                import uuid
                
                try:
                    # Try to save the actual image file
                    image_path = image_service.save_labeled_face_image(
                        unknown_face_id, person_id, person_name
                    )
                except Exception:
                    # Fallback to virtual path if image saving fails
                ...
                cursor.execute(
                    ...
                    (person_id, image_path, embedding, "unknown_labeled"),
                )
